...
PERMISSIONS = {"Admin": {"dashboard", "projects", "risk", "workforce", "resources", "attrition", "employees", "user_management", "user_management_write", "projects_write", "employees_write"},
               ...
               "Employee": {"dashboard", "projects", "workforce", "employees", "own_work_write"}}
DEMO_PERMISSIONS = {"Admin": {"dashboard", "projects", "risk", "workforce", "resources", "attrition", "employees", "user_management"},
                   ...
                   "Employee": {"dashboard", "projects", "workforce", "employees"}}
...
def login(username, password):
    ...
    payload = {"sub": user["username"], "user_id": user["id"], "employee_id": user["employee_id"], "exp": int(time.time()) + 8 * 3600}
...
def demo_login(role):
    ...
    payload = {"sub": f"demo:{role}", "role": role, "employee_id": 1 if role == "Employee" else None, "demo": True, "exp": int(time.time()) + 2 * 3600}
    raw = base64.urlsafe_b64encode(json.dumps(payload).encode()).decode()
    sig = hmac.new(SECRET.encode(), raw.encode(), hashlib.sha256).hexdigest()
    return f"{raw}.{sig}", payload
    ...
    try:
        ...
        if payload.get("demo"):
            if payload.get("role") not in ROLES:
                raise ValueError
            return {"sub": payload["sub"], "role": payload["role"], "employee_id": payload.get("employee_id"), "demo": True}
        user = find(payload.get("user_id", payload["sub"]))
        if not user or not user["is_active"]:
            raise ValueError
        return {"sub": user["username"], "user_id": user["id"], "employee_id": user["employee_id"], "role": user["role"], "demo": False}
    except Exception:
